...
void FlatpakApi::SetUp(flutter::BinaryMessenger* binary_messenger,
      ...
      channel.SetMessageHandler(
          [api](const EncodableValue& message,
            ...
            try {
              ...
              EncodableList wrapped;
              wrapped.emplace_back(EncodableValue(std::move(output).TakeValue()));
              reply(EncodableValue(std::move(wrapped)));
            } catch (const std::exception& exception) {
              reply(WrapError(exception.what()));
            }
  ...
  }
  {
    BasicMessageChannel<> channel(
        binary_messenger,
        "dev.flutter.pigeon.flatpak_flutter.FlatpakApi.setupEventChannel" +
            prepended_suffix,
        &GetCodec());
    if (api != nullptr) {
      channel.SetMessageHandler(
          [api](const EncodableValue& message,
            ...
            try {
              ...
              const auto& app_id_arg =
                  std::get<std::string>(encodable_app_id_arg);
              api->SetupEventChannel(
                  app_id_arg, [reply](std::optional<FlutterError>&& output) {
                    if (output.has_value()) {
