...
// Names this plugin was written against, before upstream renamed the pigeon
// classes. Aliases rather than a rename across the implementation: a rename
// that is only a rename costs nothing here, and anything that is *not* just a
// rename still fails to compile, which is the point.
class CoreFirebaseOptions;
class CoreInitializeResponse;
using PigeonFirebaseOptions = CoreFirebaseOptions;
using PigeonInitializeResponse = CoreInitializeResponse;
