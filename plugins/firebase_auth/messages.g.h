...
// Names this plugin was written against, before upstream renamed the
// pigeon payload classes Pigeon<X> -> Internal<X>. Aliases rather than a
// rename across the implementation: a rename that is only a rename costs
// nothing here, and anything that is *not* just a rename still fails to
...
using PigeonActionCodeInfo = InternalActionCodeInfo;
using PigeonActionCodeInfoData = InternalActionCodeInfoData;
using PigeonActionCodeSettings = InternalActionCodeSettings;
using PigeonAdditionalUserInfo = InternalAdditionalUserInfo;
using PigeonAuthCredential = InternalAuthCredential;
using PigeonFirebaseAuthSettings = InternalFirebaseAuthSettings;
using PigeonIdTokenResult = InternalIdTokenResult;
using PigeonMultiFactorInfo = InternalMultiFactorInfo;
using PigeonMultiFactorSession = InternalMultiFactorSession;
using PigeonPhoneMultiFactorAssertion = InternalPhoneMultiFactorAssertion;
using PigeonSignInProvider = InternalSignInProvider;
using PigeonTotpSecret = InternalTotpSecret;
using PigeonUserCredential = InternalUserCredential;
using PigeonUserDetails = InternalUserDetails;
using PigeonUserInfo = InternalUserInfo;
using PigeonUserProfile = InternalUserProfile;
using PigeonVerifyPhoneNumberRequest = InternalVerifyPhoneNumberRequest;
...
class InternalUserInfo {
 public:
  // Constructs an object setting all non-nullable fields.
  explicit InternalUserInfo(const std::string& uid,
                            bool is_anonymous,
