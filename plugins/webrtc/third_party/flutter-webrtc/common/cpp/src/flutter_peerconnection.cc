...
EncodableMap rtpParametersToMap(
    const libwebrtc::scoped_refptr<libwebrtc::RTCRtpParameters>&
        rtpParameters) {
  EncodableMap info;
  ...
  for (const scoped_refptr<libwebrtc::RTCRtpExtension>& extension :
    ...
    headerExtensions.emplace_back(map);
  }
  ...
  for (const scoped_refptr<libwebrtc::RTCRtpEncodingParameters>& encoding :
  ...
  for (const scoped_refptr<RTCRtpCodecParameters>& codec :
    ...
    for (const auto& item : parameters.std_vector()) {
...
EncodableMap dtmfSenderToMap(const scoped_refptr<RTCDtmfSender>& dtmfSender,
                             std::string id) {
  EncodableMap info;
...
EncodableMap mediaTrackToMap(
    const libwebrtc::scoped_refptr<libwebrtc::RTCMediaTrack>& track) {
  EncodableMap info;
  ...
  if ("video" == kind) {
    info[EncodableValue("readyState")] =
        EncodableValue(dynamic_cast<RTCVideoTrack*>(track.get())->state());
    info[EncodableValue("label")] = EncodableValue("video");
  } else if ("audio" == kind) {
    info[EncodableValue("readyState")] =
        EncodableValue(dynamic_cast<RTCAudioTrack*>(track.get())->state());
...
EncodableMap rtpSenderToMap(
    const libwebrtc::scoped_refptr<libwebrtc::RTCRtpSender>& sender) {
  EncodableMap info;
...
EncodableMap rtpReceiverToMap(
    const libwebrtc::scoped_refptr<libwebrtc::RTCRtpReceiver>& receiver) {
  EncodableMap info;
...
EncodableMap transceiverToMap(
    const scoped_refptr<RTCRtpTransceiver>& transceiver) {
  EncodableMap info;
...
EncodableMap mediaStreamToMap(const scoped_refptr<RTCMediaStream>& stream,
                              std::string id) {
  EncodableMap params;
  ...
  for (const scoped_refptr<RTCAudioTrack>& val : audio_tracks.std_vector()) {
    audioTracks.emplace_back(mediaTrackToMap(val));
  }
  params[EncodableValue("audioTracks")] = EncodableValue(audioTracks);

  EncodableList videoTracks;
  auto video_tracks = stream->video_tracks();
  for (const scoped_refptr<RTCVideoTrack>& val : video_tracks.std_vector()) {
    videoTracks.emplace_back(mediaTrackToMap(val));
  }
...
void FlutterPeerConnection::CreateRTCPeerConnection(
  ...
  // ihs::log::debug(" configuration = {}", configurationMap.StringValue());
  base_->ParseRTCConfiguration(configurationMap, base_->configuration_);
  // ihs::log::debug(" constraints = {}", constraintsMap.StringValue());
...
void FlutterPeerConnection::CreateOffer(
  ...
  pc->CreateOffer(
      [result_ptr](const libwebrtc::string& sdp,
...
FlutterPeerConnection::mapToRtpTransceiverInit(const EncodableMap& params) {
  ...
  for (const auto& item : streamIds) {
    auto id = GetValue<std::string>(item);
    stream_ids.emplace_back(id.c_str());
  }
  ...
  for (const EncodableValue& value : sendEncodings) {
...
FlutterPeerConnection::mapToEncoding(const EncodableMap& params) {
  ...
  if (!value.IsNull()) {
    const auto rid = GetValue<std::string>(value);
...
void FlutterPeerConnection::GetTransceivers(
  ...
  for (const scoped_refptr<RTCRtpTransceiver>& transceiver :
       transceivers.std_vector()) {
    info.emplace_back(transceiverToMap(transceiver));
  }
...
void FlutterPeerConnection::GetReceivers(
  ...
  for (const scoped_refptr<RTCRtpReceiver>& receiver : receivers.std_vector()) {
    info.emplace_back(rtpReceiverToMap(receiver));
  }
...
void FlutterPeerConnection::RtpSenderSetTrack(
    ...
    std::unique_ptr<MethodResultProxy> result) {
  std::shared_ptr<MethodResultProxy> result_ptr(result.release());
  auto sender = base_->GetRtpSenderById(pc, std::move(rtpSenderId));
...
scoped_refptr<RTCRtpParameters> FlutterPeerConnection::updateRtpParameters(
  ...
  for (const auto& param : params.std_vector()) {
    if (encoding != encodings.end()) {
      auto map = GetValue<EncodableMap>(*encoding);
      ...
      if (!value.IsNull()) {
        param->set_ssrc(static_cast<uint32_t>(GetValue<int>(value)));
      }
  ...
  if (!value.IsNull()) {
    const auto degradationPreference = GetValue<std::string>(value);
...
void FlutterPeerConnection::CaptureFrame(
    ...
    std::string path,
    std::unique_ptr<MethodResultProxy> result) {
  FlutterFrameCapturer capturer(track, std::move(path));
...
scoped_refptr<RTCRtpTransceiver> FlutterPeerConnection::getRtpTransceiverById(
  ...
  for (const scoped_refptr<RTCRtpTransceiver>& transceiver :
    ...
    if (nullptr == result.get() &&
        transceiver->transceiver_id().std_string() == id) {
      result = transceiver;
    }
  }
  return result;
...
void FlutterPeerConnection::RtpTransceiverSetCodecPreferences(
  ...
  for (const auto& codec : codecs) {
    ...
    if (!codecSdpFmtpLine.empty() && !codecSdpFmtpLine.empty())
...
void FlutterPeerConnection::GetSenders(
  ...
  for (const scoped_refptr<RTCRtpSender>& sender : senders.std_vector()) {
    info.emplace_back(rtpSenderToMap(sender));
  }
...
void FlutterPeerConnection::GetStats(
  ...
  if (track != nullptr && !track_id.empty()) {
    auto receivers = pc->receivers();
    for (const auto& receiver : receivers.std_vector()) {
        ...
        pc->GetStats(
            ...
            [result_ptr](const vector<scoped_refptr<MediaRTCStats>>& reports) {
              ...
              for (size_t i = 0; i < reports.size(); i++) {
                list.emplace_back(statsToMap(reports[i]));
              }
              EncodableMap params;
    ...
    auto senders = pc->senders();
    for (const auto& sender : senders.std_vector()) {
...
void FlutterPeerConnection::MediaStreamAddTrack(
  ...
  std::string kind = track->kind().std_string();
  if ("audio" == kind) {
    stream->AddTrack(dynamic_cast<RTCAudioTrack*>(track.get()));
  } else if ("video" == kind) {
    stream->AddTrack(dynamic_cast<RTCVideoTrack*>(track.get()));
  }
...
void FlutterPeerConnection::MediaStreamRemoveTrack(
  ...
  std::string kind = track->kind().std_string();
  if ("audio" == kind) {
    stream->RemoveTrack(dynamic_cast<RTCAudioTrack*>(track.get()));
  } else if ("video" == kind) {
    stream->RemoveTrack(dynamic_cast<RTCVideoTrack*>(track.get()));
  }
...
void FlutterPeerConnection::RemoveTrack(
    ...
    std::string senderId,
    std::unique_ptr<MethodResultProxy> result) {
  auto sender = base_->GetRtpSenderById(pc, std::move(senderId));
  if (nullptr == sender.get()) {
...
void FlutterPeerConnectionObserver::OnAddStream(
  ...
  EncodableList audioTracks;
  auto audio_tracks = stream->audio_tracks();
  for (const scoped_refptr<RTCAudioTrack>& track : audio_tracks.std_vector()) {
    ...
    audioTracks.emplace_back(audioTrack);
  }
  params[EncodableValue("audioTracks")] = EncodableValue(audioTracks);

  EncodableList videoTracks;
  auto video_tracks = stream->video_tracks();
  for (const scoped_refptr<RTCVideoTrack>& track : video_tracks.std_vector()) {
    ...
    videoTracks.emplace_back(videoTrack);
  }
...
void FlutterPeerConnectionObserver::OnAddTrack(
  ...
  for (const scoped_refptr<RTCMediaStream>& stream : streams.std_vector()) {
...
void FlutterPeerConnectionObserver::OnTrack(
  ...
  for (const scoped_refptr<RTCMediaStream>& item : streams.std_vector()) {
    streams_info.emplace_back(mediaStreamToMap(item, id_));
  }
...
scoped_refptr<RTCMediaTrack> FlutterPeerConnectionObserver::MediaTrackForId(
    const std::string& id) {
  for (auto& remote_stream : remote_streams_) {
    auto remoteStream = remote_stream.second;
