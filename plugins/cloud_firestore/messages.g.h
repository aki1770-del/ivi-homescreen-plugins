...
enum class InternalTransactionResult;
using PigeonTransactionResult = InternalTransactionResult;
enum class InternalTransactionType;
using PigeonTransactionType = InternalTransactionType;
class PigeonInternalCodecSerializer;
using FirebaseFirestoreHostApiCodecSerializer = PigeonInternalCodecSerializer;
...
using PigeonDocumentChange = InternalDocumentChange;
using PigeonDocumentOption = InternalDocumentOption;
using PigeonDocumentSnapshot = InternalDocumentSnapshot;
using PigeonFirebaseSettings = InternalFirebaseSettings;
using PigeonGetOptions = InternalGetOptions;
using PigeonQueryParameters = InternalQueryParameters;
using PigeonQuerySnapshot = InternalQuerySnapshot;
using PigeonSnapshotMetadata = InternalSnapshotMetadata;
using PigeonTransactionCommand = InternalTransactionCommand;
...
class FirebaseFirestoreHostApi {
 public:
  ...
  virtual void QueryGet(
      ...
      const InternalQueryParameters& parameters,
      const InternalGetOptions& options,
      std::function<void(ErrorOr<InternalQuerySnapshot> reply)> result) = 0;
