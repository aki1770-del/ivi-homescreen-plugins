// POST-GENERATION EDIT: pigeon names both an enum and a HostApi method
// PersistenceCacheIndexManagerRequest. Inside the API's own scope the method
// name hides the type, so the generated std::any_cast<const
// PersistenceCacheIndexManagerRequest&> does not compile; it is qualified with
// the namespace below. Re-apply after regenerating.
//
...
bool InternalSnapshotMetadata::is_from_cache() const {
...
const std::string& InternalDocumentSnapshot::path() const {
  return path_;
}
...
const DocumentChangeType& InternalDocumentChange::type() const {
  return type_;
}
...
int64_t InternalDocumentChange::old_index() const {
  return old_index_;
}
...
int64_t InternalDocumentChange::new_index() const {
  return new_index_;
}
...
const Source& InternalGetOptions::source() const {
  return source_;
}
...
size_t PigeonInternalDeepHash(const InternalGetOptions& v) {
  return v.Hash();
}
...
void InternalDocumentOption::set_merge(bool value_arg) {
  merge_ = value_arg;
}
...
InternalTransactionCommand::InternalTransactionCommand(
    const InternalTransactionType& type,
    const std::string& path)
...
const std::string& InternalTransactionCommand::path() const {
  return path_;
}
...
size_t PigeonInternalDeepHash(const AggregateQuery& v) {
  return v.Hash();
}
      ...
      channel.SetMessageHandler(
          [api](const EncodableValue& message,
            ...
            try {
              ...
              const auto& request_arg =
                  std::any_cast<const cloud_firestore_linux::
