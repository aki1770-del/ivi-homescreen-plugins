  ...
  }
  {
    ...
    if (api != nullptr) {
      channel.SetMessageHandler([api](const EncodableValue& message, const flutter::MessageReply<EncodableValue>& reply) {
        try {
          const auto& args = std::get<EncodableList>(message);
          const auto& encodable_camera_idd_arg = args.at(0);
          if (encodable_camera_idd_arg.IsNull()) {
            reply(WrapError("camera_idd_arg unexpectedly null."));
            return;
          }
          const int64_t camera_idd_arg = encodable_camera_idd_arg.LongValue();
          api->ResumePreview(camera_idd_arg, [reply](std::optional<FlutterError>&& output) {
            if (output.has_value()) {
              reply(WrapError(output.value()));
              return;
            }
