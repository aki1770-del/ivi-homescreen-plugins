...
std::string CameraSession::takePicture() {
  ...
  spdlog::debug("[camera_plugin] filename: {}", filename.value());
...
double CameraSession::setExposureOffset(double offset) {
  ...
  SPDLOG_DEBUG("[camera_plugin] setBightness: value: {}", offset);

  //auto exposureControl = camera_->controls().get<libcamera::controls::ExposureTime>();
/*
  std::unique_ptr<libcamera::Request> request = camera_->createRequest();
  if (!request) {
    // handle error
    SPDLOG_DEBUG("[camera_plugin] can't create a request...");
  }

  libcamera::ControlList &controls = request->controls();
  controls.set(libcamera::controls::Brightness, 100);

  if(camera_->queueRequest(request.get())<0) {
    SPDLOG_DEBUG("[camera_plugin] can't queue a request...: ");
...
/*
  ...
  for (const auto& [id, info] : camera_->controls()) {
    if ("ExposureTime" == id->name() &&
        id->type() == libcamera::ControlTypeInteger32) {
      //result = info.get<int32_t>();
      //result = info.min().get<int32_t>();
      //result = info.def().get<int32_t>();
      SPDLOG_DEBUG("[camera_plugin] getMinExposureOffset: offset: {}", info.toString());
  ...
  */
  ...
  request->controls().set(libcamera::controls::ExposureTime, offset);
  mRequests.push_back(std::move(request));
  return offset;
 */
}
